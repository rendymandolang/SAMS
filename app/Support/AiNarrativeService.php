...
class AiNarrativeService
{
    public function generate(int $companyId): array
    {
        ...
        $budget = $s['budget']; $used = $budget['committed'] + $budget['actual'];
        $percent = $budget['allocated'] > 0 ? round($used / $budget['allocated'] * 100, 1) : 0;
        $stock = $s['stock_forecasts'][0] ?? null; $supplier = $s['supplier_risks'][0] ?? null; $asset = $s['maintenance_predictions'][0] ?? null;
