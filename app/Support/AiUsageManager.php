...
class AiUsageManager
{
    ...
    public function ensureAllowed(int $companyId, bool $external = false): void
    {
        ...
        if (! $settings->is_enabled) throw ValidationException::withMessages(['ai' => 'AI dinonaktifkan untuk perusahaan ini.']);
        if ($external && ! $settings->allow_external_provider) throw ValidationException::withMessages(['ai' => 'Provider eksternal belum diizinkan untuk perusahaan ini.']);
        if ($usage['requests'] >= $settings->monthly_request_limit) throw ValidationException::withMessages(['ai' => 'Batas request AI bulanan sudah tercapai.']);
        if ($usage['tokens'] >= $settings->monthly_token_limit) throw ValidationException::withMessages(['ai' => 'Batas token AI bulanan sudah tercapai.']);
    }
}
