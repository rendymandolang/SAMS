...
class SupplierCatalogScanner
{
    ...
    private function spreadsheet(string $path): array
    {
        ...
        [$headerIndex, $map] = $this->detectHeader($rows); if ($headerIndex === null) throw new RuntimeException('Header nama produk dan harga tidak ditemukan.');
        $result=[]; foreach (array_slice($rows, $headerIndex+1, 5000, true) as $index=>$row) { $item=$this->mapRow($row,$map,$index+1); if($item) $result[]=$item; }
    ...
    {
        $text=(new Parser())->parseFile($path)->getText(); $items=[]; $lineNo=0; $previousText='';
        foreach (preg_split('/\R/u',$text) as $line) {
            $lineNo++; $line=trim(preg_replace('/\s+/u',' ',$line)); if($line==='')continue;
            if (preg_match('/^(?:Rp\.?\s*)?([0-9][0-9\.\,]{2,})\s*$/iu',$line,$m) && mb_strlen($previousText)>=2) { $name=$previousText;$price=$this->number($m[1]); }
            elseif (preg_match('/^(.*?)\s+(?:Rp\.?\s*)?([0-9][0-9\.\,]{2,})\s*$/iu',$line,$m) && mb_strlen(trim($m[1]))>=2) { $name=trim($m[1]);$price=$this->number($m[2]); }
            else { $previousText=$line; continue; }
            if($price>0)$items[]=$this->build(['name'=>$name,'price'=>$price,'unit'=>null,'source_row'=>$lineNo,'raw'=>['line'=>$line]],.55);$previousText='';
        }
    ...
    {
        foreach(array_slice($rows,0,10,true) as $i=>$row){$map=[];foreach($row as $col=>$header){$h=mb_strtolower(trim((string)$header));foreach($this->aliases as $field=>$aliases)if(in_array($h,$aliases,true))$map[$field]=$col;}if(isset($map['name'],$map['price']))return[$i,$map];}return[null,[]];
    }
    private function mapRow(array $row,array $map,int $sourceRow): ?array
    {
        $name=trim((string)($row[$map['name']]??''));$price=$this->number($row[$map['price']]??0);if($name===''||$price<=0)return null;
        $data=[];foreach($map as $field=>$col)$data[$field]=$row[$col]??null;$data['name']=$name;$data['price']=$price;$data['source_row']=$sourceRow;$data['raw']=$data;return $this->build($data,.85);
    }
    private function build(array $data,float $confidence): array
    {
        $unit=app(CatalogUnitNormalizer::class)->parse(isset($data['unit'])?(string)$data['unit']:null,$data['name']);$normalized=max(.000001,$unit['normalized_quantity']);
