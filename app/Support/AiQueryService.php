...
class AiQueryService
{
    ...
    private function budget(array $s): string { $b=$s['budget']; $remaining=$b['allocated']-$b['committed']-$b['actual']; return 'Budget teralokasi Rp '.number_format($b['allocated'],0,',','.').', committed Rp '.number_format($b['committed'],0,',','.').', actual Rp '.number_format($b['actual'],0,',','.').', dan tersisa Rp '.number_format($remaining,0,',','.').'.'; }
    private function stock(array $s): string { $top=$s['stock_forecasts'][0]??null; return $top ? "Prioritas reorder: {$top['sku']} sebanyak {$top['recommended_reorder']} unit; days cover ".($top['days_cover']??'belum cukup data').'.' : 'Belum ada item yang memiliki pola pemakaian atau kebutuhan reorder terdeteksi.'; }
    private function supplier(array $s): string { $top=$s['supplier_risks'][0]??null; return $top ? "Supplier dengan risiko tertinggi: {$top['name']} dengan skor {$top['risk_score']}/100 ({$top['risk_level']})." : 'Belum ada histori transaksi supplier yang cukup untuk dinilai.'; }
    private function maintenance(array $s): string { $top=$s['maintenance_predictions'][0]??null; return $top ? "Aset prioritas: {$top['asset_number']} dengan skor {$top['risk_score']}/100 dan prediksi maintenance {$top['predicted_maintenance_date']} (confidence {$top['confidence']})." : 'Belum ada aset aktif yang dapat diprediksi.'; }
}
