...
class SupplierCatalogController extends Controller
{
    ...
    {
        $company=$context->current();$v=$request->validate(['supplier_id'=>['required','integer'],'name'=>['required','string','max:255'],'currency'=>['required','string','size:3'],'valid_from'=>['nullable','date'],'valid_until'=>['nullable','date','after_or_equal:valid_from'],'catalog_file'=>['required','file','max:20480','mimes:csv,txt,xlsx,xls,pdf']]);
        abort_unless(DB::table('suppliers')->where('company_id',$company->id)->where('id',$v['supplier_id'])->exists(),404);$file=$request->file('catalog_file');$path=$file->store('supplier-catalogs/'.$company->public_id);
        ...
        try{$scan=$scanner->scan(Storage::path($path),strtolower($file->getClientOriginalExtension()));DB::transaction(function()use($scan,$catalogId){foreach($scan['items'] as $item)DB::table('supplier_catalog_items')->insert(['supplier_catalog_id'=>$catalogId,...collect($item)->except('raw_data')->all(),'raw_data'=>json_encode($item['raw_data']??[]),'status'=>'staged','created_at'=>now(),'updated_at'=>now()]);DB::table('supplier_catalogs')->where('id',$catalogId)->update(['status'=>'scanned','row_count'=>count($scan['items']),'scan_summary'=>json_encode($scan['summary']),'updated_at'=>now()]);});AuditLogger::log('supplier_catalog_scanned','supplier_catalog',$catalogId,null,['rows'=>count($scan['items'])],(int)$company->id);}catch(Throwable $e){DB::table('supplier_catalogs')->where('id',$catalogId)->update(['status'=>'failed','error_message'=>mb_substr($e->getMessage(),0,2000),'updated_at'=>now()]);report($e);return redirect()->route('supplier-catalogs.show',$catalogId)->withErrors(['catalog_file'=>'Scan gagal: '.$e->getMessage()]);}
    ...
    public function show(int $catalog,CompanyContext $context):View{$company=$context->current();$catalogRow=DB::table('supplier_catalogs')->join('suppliers','suppliers.id','=','supplier_catalogs.supplier_id')->where('supplier_catalogs.company_id',$company->id)->where('supplier_catalogs.id',$catalog)->select('supplier_catalogs.*','suppliers.name as supplier_name')->first();abort_unless($catalogRow,404);$items=DB::table('supplier_catalog_items')->where('supplier_catalog_id',$catalog)->orderBy('source_row')->paginate(100);return view('supplier-catalogs.show',['catalog'=>$catalogRow,'items'=>$items]);}
