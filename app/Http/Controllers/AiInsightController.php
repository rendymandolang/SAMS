...
class AiInsightController extends Controller
{
    public function index(CompanyContext $context, AiInsightService $service, AiUsageManager $usageManager, LiveBankRateService $bankRateService): View
    {
        ...
        $runs = DB::table('ai_insight_runs')->where('company_id', $company->id)->orderByDesc('id')->limit(10)->get()
            ->map(function (object $run): object {
                $run->output = json_decode($run->output, true) ?: [];
    ...
    public function query(Request $request, CompanyContext $context, AiQueryService $service): RedirectResponse
    {
    ...
    public function updateSettings(Request $request, CompanyContext $context, AiUsageManager $usageManager): RedirectResponse
    {
        ...
        AuditLogger::log('ai_settings_updated', 'ai_company_setting', (int) $settings->id, null, $validated + ['is_enabled' => $request->boolean('is_enabled'), 'allow_external_provider' => $request->boolean('allow_external_provider')], (int) $context->id());
