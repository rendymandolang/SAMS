...
class SupplierCatalogTest extends TestCase
{
    ...
    {
        Storage::fake('local'); $this->seed();
        $user=User::query()->where('email','admin@sams.local')->firstOrFail();$supplier=DB::table('suppliers')->firstOrFail();
        ...
        $catalog=DB::table('supplier_catalogs')->firstOrFail();$response->assertRedirect('/supplier-catalogs/'.$catalog->id);
    ...
    {
        $sheet=new Spreadsheet();$sheet->getActiveSheet()->fromArray([['Product Code','Product Name','Retail Price','Pack Size','Brand','Category'],['CHR-01','Kursi Kantor Ergonomic',1750000,'1 pcs','Furniture Test','Furniture'],['PEN-12','Pulpen Biru 12 pcs',48000,'12 pcs','Office Test','ATK']]);
        $path=tempnam(sys_get_temp_dir(),'sams-catalog-').'.xlsx';(new Xlsx($sheet))->save($path);$scan=app(SupplierCatalogScanner::class)->scan($path,'xlsx');@unlink($path);
        $this->assertCount(2,$scan['items']);$this->assertSame('PCS',$scan['items'][0]['normalized_unit']);$this->assertEquals(1750000,$scan['items'][0]['normalized_unit_price']);$this->assertEquals(4000,$scan['items'][1]['normalized_unit_price']);
    }

    public function test_unpublished_catalog_is_not_used_and_staff_cannot_upload(): void
    {
        Storage::fake('local');$this->seed();$staff=User::query()->where('email','staff@sams.local')->firstOrFail();$supplier=DB::table('suppliers')->firstOrFail();
    ...
    {
        $path=tempnam(sys_get_temp_dir(),'sams-pdf-').'.pdf';file_put_contents($path,$this->simplePdf(['Salmon Fillet Fresh 500 gr Rp 200.000','Kursi Kantor Ergonomic 1 pcs','Rp 1.750.000']));
        $scan=app(SupplierCatalogScanner::class)->scan($path,'pdf');@unlink($path);
        $this->assertCount(2,$scan['items']);$this->assertSame('Salmon Fillet Fresh 500 gr',$scan['items'][0]['source_name']);$this->assertEquals(400000,$scan['items'][0]['normalized_unit_price']);$this->assertSame('Kursi Kantor Ergonomic 1 pcs',$scan['items'][1]['source_name']);$this->assertEquals(1750000,$scan['items'][1]['normalized_unit_price']);$this->assertFalse($scan['summary']['requires_ocr']);
    }

    private function simplePdf(array $lines): string
    {
        $content="BT /F1 12 Tf 72 760 Td ";foreach($lines as $index=>$line){if($index>0)$content.="0 -22 Td ";$content.='('.str_replace(['\\','(',')'],['\\\\','\\(','\\)'],$line).") Tj ";}$content.='ET';
