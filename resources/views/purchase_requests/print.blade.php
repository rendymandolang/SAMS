...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .right { text-align:right; } .center { text-align:center; } .total-row td { font-weight:800; background:#fafafa; }
...
<body>
    <div class="screen-toolbar">
        <a class="button secondary" href="{{ route('purchase-requests.show', $header->id) }}">Kembali</a>
    ...
    <main class="page">
        <section class="header">
            <div class="brand">
                ...
                <div>
                    ...
                    <p class="muted" style="margin-bottom:4px;">{{ $branch?->name ?? $header->branch_name }}</p>
        ...
        <section class="grid">
            <div class="box">
                ...
                <table class="info-table">
                    <tr><td>Departemen</td><td><strong>{{ $header->department_name }}</strong></td></tr>
                    <tr><td>Peminta</td><td>{{ $header->requester_name }}</td></tr>
                    <tr><td>Cabang</td><td>{{ $header->branch_name }}</td></tr>
                    <tr><td>Prioritas</td><td>{{ ucfirst($header->priority) }}</td></tr>
                </table>
            </div>
            ...
            <div class="box">
                ...
                <table class="info-table">
                    <tr><td>No PR</td><td><strong>{{ $header->document_number }}</strong></td></tr>
                    <tr><td>Tanggal Request</td><td>{{ \Illuminate\Support\Carbon::parse($header->request_date)->format('d M Y') }}</td></tr>
                    <tr><td>Dibutuhkan</td><td>{{ $header->required_date ? \Illuminate\Support\Carbon::parse($header->required_date)->format('d M Y') : '-' }}</td></tr>
                    <tr><td>Total Estimasi</td><td>Rp {{ number_format((float) $header->estimated_total, 0, ',', '.') }}</td></tr>
                </table>
            </div>
        </section>
        ...
        <table class="items">
            ...
            <tr>
                ...
                <th>Satuan</th>
                <th>Budget</th>
                <th class="right">Harga Estimasi</th>
            ...
            @foreach ($items as $index => $item)
                <tr>
                    ...
                    <td class="right">Rp {{ number_format((float) $item->estimated_total, 0, ',', '.') }}</td>
                </tr>
            @endforeach
            <tr class="total-row"><td colspan="7" class="right">Grand Total Estimasi</td><td class="right">Rp {{ number_format((float) $header->estimated_total, 0, ',', '.') }}</td></tr>
            </tbody>
        </table>

        <section class="notes">
            <strong>Tujuan / Catatan</strong>
            <p style="margin:8px 0 0;line-height:1.6;">{{ $header->purpose ?: '-' }}</p>
        </section>

        <section class="signatures">
            <div class="signature-box"><strong>Diajukan oleh</strong><div class="signature-line">{{ $header->requester_name }}</div></div>
            <div class="signature-box"><strong>Diperiksa oleh</strong><div class="signature-line">Department Head</div></div>
            <div class="signature-box"><strong>Disetujui oleh</strong><div class="signature-line">Approver / Finance</div></div>
        </section>

        <p class="footer">Dicetak dari SAMS pada {{ now()->format('d M Y H:i') }} · Purchase Request menjadi dasar pembuatan Purchase Order setelah approved.</p>
    </main>
</body>
</html>
