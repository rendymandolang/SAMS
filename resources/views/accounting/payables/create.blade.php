        ...
        <form method="POST" action="{{ route('accounting.payables.store') }}">@csrf
            <section class="card" style="margin-bottom:18px">
                ...
                <div class="form-grid">
                    ...
                    <label class="field"><span class="label">Accounts Payable Account</span><select class="input" name="ap_account_id" required>@foreach($liabilityAccounts as $account)<option value="{{ $account->id }}" @selected(old('ap_account_id', $postingRules['payable'] ?? null) == $account->id)>{{ $account->code }} · {{ $account->name }}</option>@endforeach</select></label>
                    <label class="field"><span class="label">Purchase Tax Code</span><select class="input" name="tax_code_id"><option value="">Manual / No tax</option>@foreach($taxCodes as $tax)<option value="{{ $tax->id }}">{{ $tax->code }} · {{ $tax->name }} ({{ number_format($tax->rate_percent, 2) }}%)</option>@endforeach</select></label>
                    <label class="field"><span class="label">Withholding Tax</span><select class="input" name="withholding_tax_code_id"><option value="">No withholding</option>@foreach($withholdingCodes as $tax)<option value="{{ $tax->id }}">{{ $tax->code }} · {{ $tax->name }} ({{ number_format($tax->rate_percent, 2) }}%)</option>@endforeach</select></label>
            ...
            <section class="card">
                ...
                <div class="table-wrap"><table><thead><tr><th>Account</th><th>PO Item</th><th>Goods Receipt</th><th>Department</th><th>Description</th><th>Qty</th><th>Unit Price</th></tr></thead><tbody>
                    @for($index = 0; $index < 8; $index++)
                        <tr><td><select class="input" name="lines[{{ $index }}][gl_account_id]" @if($index === 0) required @endif><option value="">Select account</option>@foreach($debitAccounts as $account)<option value="{{ $account->id }}" @selected(($postingRules['expense'] ?? null) == $account->id)>{{ $account->code }} · {{ $account->name }}</option>@endforeach</select></td><td><select class="input" name="lines[{{ $index }}][purchase_order_item_id]"><option value="">Non-PO</option>@foreach($poItems as $item)<option value="{{ $item->id }}">{{ $item->po_number }} · {{ $item->item_name }} · {{ number_format($item->unit_price,2,',','.') }}</option>@endforeach</select></td><td><select class="input" name="lines[{{ $index }}][goods_receipt_item_id]"><option value="">No receipt</option>@foreach($receiptItems as $item)<option value="{{ $item->id }}">{{ $item->receipt_number }} · {{ $item->item_name }} · {{ number_format($item->accepted_quantity,4,',','.') }}</option>@endforeach</select></td><td><select class="input" name="lines[{{ $index }}][department_id]"><option value="">-</option>@foreach($departments as $department)<option value="{{ $department->id }}">{{ $department->name }}</option>@endforeach</select></td><td><input class="input" name="lines[{{ $index }}][description]" @if($index === 0) required @endif></td><td><input class="input" type="number" name="lines[{{ $index }}][quantity]" min="0.0001" step="0.0001" value="{{ $index === 0 ? 1 : '' }}"></td><td><input class="input" type="number" name="lines[{{ $index }}][unit_price]" min="0.0001" step="0.01"></td></tr>
                    @endfor
                </tbody></table></div>
