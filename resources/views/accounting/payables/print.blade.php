...
<main class="print-sheet">
    ...
    <table class="print-table">
        <thead><tr><th>#</th><th>Account / Department</th><th>Description</th><th>Qty</th><th>Unit Price</th><th>Amount</th></tr></thead>
        <tbody>
        @foreach($lines as $line)
            <tr><td>{{ $line->line_number }}</td><td>{{ $line->account_code }} · {{ $line->account_name }}<br>{{ $line->department_name ?: '-' }}</td><td>{{ $line->description }}</td><td class="number">{{ number_format($line->quantity, 4, ',', '.') }}</td><td class="number">{{ number_format($line->foreign_unit_price, 2, ',', '.') }}</td><td class="number">{{ number_format($line->foreign_amount, 2, ',', '.') }}</td></tr>
        @endforeach
        <tr><th colspan="5" class="number">Subtotal</th><th class="number">{{ number_format($invoice->foreign_subtotal, 2, ',', '.') }}</th></tr>
        <tr><th colspan="5" class="number">Tax</th><th class="number">{{ number_format($invoice->foreign_tax_amount, 2, ',', '.') }}</th></tr>
        <tr><th colspan="5" class="number">TOTAL {{ $invoice->currency }}</th><th class="number">{{ number_format($invoice->foreign_total_amount, 2, ',', '.') }}</th></tr>
        <tr><th colspan="5" class="number">Outstanding {{ $invoice->currency }}</th><th class="number">{{ number_format($invoice->foreign_outstanding_amount, 2, ',', '.') }}</th></tr>
        <tr><th colspan="5" class="number">Base Carrying Amount</th><th class="number">Rp {{ number_format($invoice->carrying_amount, 2, ',', '.') }}</th></tr>
        </tbody>
    </table>
