...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .info-table { width:100%; border-collapse:collapse; } .info-table td { padding:3px 0; vertical-align:top; } .info-table td:first-child { width:130px; color:var(--muted); }
...
<body>
    <div class="screen-toolbar">
        <a class="button secondary" href="{{ route('stock-opnames.show', $header->id) }}">Kembali</a>
    ...
    <main class="page">
        ...
        <section class="grid">
            <div class="box">
                ...
                <table class="info-table">
                    <tr><td>No Opname</td><td><strong>{{ $header->document_number }}</strong></td></tr>
                    <tr><td>Gudang</td><td>{{ $header->location_code }} · {{ $header->location_name }}</td></tr>
                    <tr><td>Tanggal Hitung</td><td>{{ \Illuminate\Support\Carbon::parse($header->count_date)->format('d M Y') }}</td></tr>
            ...
            <div class="box">
                <h2>Ringkasan Selisih</h2>
                <table class="info-table">
                    <tr><td>Total Item</td><td>{{ number_format($summary['line_count']) }}</td></tr>
                    <tr><td>Selisih Plus</td><td>{{ number_format((float) $summary['positive_variance'], 2, ',', '.') }}</td></tr>
                    <tr><td>Selisih Minus</td><td>{{ number_format((float) $summary['negative_variance'], 2, ',', '.') }}</td></tr>
                    <tr><td>Nilai Selisih</td><td>Rp {{ number_format((float) $summary['variance_value'], 0, ',', '.') }}</td></tr>
                </table>
            </div>
        </section>
        ...
        <table class="items">
            ...
            @foreach ($items as $index => $item)
                ...
                <tr>
                    ...
                    <td class="right">{{ number_format((float) $item->system_quantity, 2, ',', '.') }}</td>
                    <td class="right">{{ $item->counted_quantity === null ? '-' : number_format((float) $item->counted_quantity, 2, ',', '.') }}</td>
                    <td class="right">{{ number_format((float) $item->variance_quantity, 2, ',', '.') }}</td>
                    <td>{{ $item->unit_code }}</td>
                    <td class="right">Rp {{ number_format((float) $item->unit_cost, 0, ',', '.') }}</td>
                    <td class="right">Rp {{ number_format($varianceValue, 0, ',', '.') }}</td>
                    <td>{{ $item->notes ?: '-' }}</td>
                </tr>
            @endforeach
            </tbody>
        ...
        <section class="signatures">
            <div class="signature-box"><strong>Dihitung oleh</strong><div class="signature-line">Stock Counter</div></div>
            <div class="signature-box"><strong>Diperiksa oleh</strong><div class="signature-line">Warehouse Lead</div></div>
            <div class="signature-box"><strong>Disetujui oleh</strong><div class="signature-line">{{ $header->creator_name }}</div></div>
        </section>

        <p class="footer">Dicetak dari SAMS pada {{ now()->format('d M Y H:i') }} · Stock Opname posted akan membuat adjustment stock movement untuk selisih plus/minus.</p>
    </main>
</body>
</html>
