...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('gl_accounts', function (Blueprint $t) {
            ...
            $t->boolean('allow_posting')->default(true);
        ...
        Schema::create('journal_entries', function (Blueprint $t) {
            ...
            $t->string('document_number', 60);
            $t->date('journal_date');
            $t->string('source_type', 40)->default('manual');
            $t->string('status', 20)->default('draft');
            $t->boolean('is_adjustment')->default(false);
            $t->text('memo');
            $t->decimal('total_debit', 19, 4)->default(0);
            $t->decimal('total_credit', 19, 4)->default(0);
        ...
        Schema::create('journal_entry_lines', function (Blueprint $t) {
            ...
            $t->foreignId('gl_account_id')->constrained()->restrictOnDelete();
            $t->foreignId('department_id')->nullable()->constrained()->nullOnDelete();
            $t->text('description')->nullable();
            $t->decimal('debit', 19, 4)->default(0);
            $t->decimal('credit', 19, 4)->default(0);
            $t->unsignedSmallInteger('line_number');
            $t->timestamps();
            $t->index(['gl_account_id', 'journal_entry_id']);
        });
